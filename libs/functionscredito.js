$(document).ready(function()
{
	...
	*/
	...
	$("input[name=codigo_producto]").delayPasteKeyUp(function()
	{
		...
		$.ajax({
			type: "POST",
			url: _root_ + 'credito/autocompletado',
			...
			success: function(data)
			{
				if(data != "")
				{
					...
					if(json.res == 'full')
					{
						...
						{
							html+='<a href="#" onclick="info('+json.data[datos].codigo+',\''+json.data[datos].descr_referencia+'\','+json.data[datos].precio_compra+')" class="list-group-item">';
...
function info(id_referencia,descr_referencia,precio_compra)
{
